...
const CreditNoteLinesFieldArray = (props) => {
  ...
  const { data: invoice } = useQuery({
    ...
    enabled: Boolean(invoiceInput?.id),
  })
  ...
  const handleAddLumpSumRow = () => {
    const lastRow = last<CreditNoteLine>(rows)
    return append({
      position: (lastRow?.position ?? -1) + 1,
      credit_note_id: lastRow?.credit_note_id,
      quantity: 1,
    })
  }
  ...
  return (
    <Card>
      ...
      {!isLumpSumType && !isReadOnly && invoice && (
        <CreditNoteLinesAddDialog
          invoice={invoice}
...
export default CreditNoteLinesFieldArray
